...
| 六動作效益的 Speed Loss 序列 | 統計模型推導 | 每船篩 STW 6–25、HP 船內 2–98%、風級≤5、全速時數比≥0.5；DISPLACEMENT 中位判載況，缺值只用原始 MID_DRAFT，再缺為 0.5；最早 30% 以 `STW=a+b·HP^(1/3)+c·載況` 建基準，SL 排除 -6%～45%，7 日中位數再減船內第 5 百分位並將底部雜訊歸零 | 不是外部海試／因果模型；與上方預測錐的分載況模型、篩選界線及用途不同，數值不可混稱 |
| 六動作事件證據 | 觀測後統計 | 77 筆原始事件去除複合列重複；前窗 D−60～D−3、後窗 D+7～D+55 各至少 3 bins，且事前 SL≥1.5pp 才納入回復；D+7～D+150 斜率×30 為觀測復發 | `n_used/n_total` 與中位數不是因果效果；UWC／UWC+PP 原始事件只有 6／7 筆，多為船體仍乾淨時的主動清洗 |
| 六動作分岔與回復比例 | physics-informed 情境估算 | UWI／PP／UWI+PP／UWC／UWC+PP／DD 預設移除 0／15／20／45／58／75% 當前汙損，可由使用者調整；分岔後採該船 `max(近期趨勢,0.3%/月)`，DD 才乘 0.5 | 回復比例是物理先驗，不是由小樣本觀測回復直接估得；觀測復發只顯示證據，不跨船套用 |
| 分岔燃油／成本／CO₂ 效益 | 粗略情境換算 | 平均 no-action−branch SL 差 × 燃油係數 3.0 × 該船全速日 `ME_CONSUMPTION` 中位數 × 展望日×出海比例 0.65；成本以 600 USD/MT，CO₂ 以 3.114 t/t，可調前三項 | 不是帳務、實測省油或 LCA；UWI 0% 與 no-action 完全重合且效益為零 |
