...
function NotificationManager() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await api.createNotificationSubscription({ channel, destination: channel === 'email' ? email : (discordUrl.trim() || undefined), ship_ids: shipIds })
      setEmail(''); setDiscordUrl(''); setShipIds([]); await load(); setMessage('訂閱已儲存。')
