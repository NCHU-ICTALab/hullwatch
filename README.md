...
Oi! Hullwatch 是一套船體能效監控與維護決策系統。系統從正午日報與水下維護事件建立每艘船的乾淨基準，估算船體髒污造成的 Speed Loss、額外油耗、碳排與成本，並提供維護排程、清洗投資報酬分析及 AI 顧問。
...
後端與 AI 顧問共用同一個服務層，確保儀表板和顧問回答使用相同的船隊數字。乾淨基準實驗可執行跨船 Leave-One-Ship-Out 與時間分塊驗證；102 格 production ensemble 則以模擬維護遮蔽窗口評估。各模型的實際驗證範圍與不可宣稱事項見 [`docs/product-truth-audit.md`](docs/product-truth-audit.md)。
...
- 船殼／螺旋槳比例目前是全船隊事件前後效果的 proxy，不是逐船因果歸因。
- 經濟 ROI 目前使用乾淨基準、立方律與事件比例的情境引擎，尚不是 102 格油耗 ensemble 的同模型 UWC／PP 反事實推論。
- 未掛載 artifacts 時 Docker 啟動流程會產生合成資料；正式環境必須驗證資料版本與掛載來源。

完整的資料真實性、公式、模型清單與命題符合度請參考 [`docs/product-truth-audit.md`](docs/product-truth-audit.md)。
